...
@tool("send_otp", description="Send an OTP to the user", return_direct=True)
def send_otp(
    ...
    try:
        ...
        return Command(
            update={"messages": [f"Failed to send OTP: {str(e)}"], "otp_sent": False},
        )


@tool("verify_otp", description="Verify the OTP provided by the user", return_direct=True)
def verify_otp(
    otp: str,
    state: Annotated[State, InjectedState],
    tool_call_id: Annotated[str, InjectedToolCallId],
    ...
    try:
        ...
        if not stored_otp:
            tool_message = ToolMessage(
                content="No OTP found for this phone number", tool_call_id=tool_call_id
            )
        ...
        tool_message = ToolMessage(
            content="Invalid OTP, Please enter the correct OTP",
            tool_call_id=tool_call_id,
        )
        ...
        tool_message = ToolMessage(
            content=f"OTP verification failed: {str(e)}", tool_call_id=tool_call_id
        )
...
def validate_authorization(
    ...
    if not state.authentication.is_authorized:
        ...
        return False, {"messages": msg_list}
